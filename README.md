<div align="center">

# Hi, I'm Isabella 👋

### she/her · 🐈 cats · 💻 code · 🏹 archery · 📚 manga · 🎮 games

<img src="https://readme-typing-svg.demolab.com/?lines=Welcome+to+my+GitHub!;I+build+things+with+TypeScript+%26+JavaScript;Cat+lover+%7C+Archer+%7C+Gamer+%7C+Manga+reader;Always+learning+something+new+%E2%9C%A8&font=Fira%20Code&center=true&width=560&height=45&color=A78BFA&vCenter=true&size=22" alt="Typing animation" />

</div>

---

### 🌟 About me

I'm a developer who likes turning ideas into small, working things, and then telling people about them here. When I'm not coding you'll usually find me practicing archery, deep in a manga, gaming, or being supervised by a cat.

- ⚡ Passions: cats, programming, archery, manga, and gaming
- 🌐 Check out my latest full projects: **[planye](https://planye.com/)** and **[habity.care](https://habity.care/)**
- 📝 More about me on my personal site: **[Riquetti Studio](http://riquetti.com.br/)** and **[isabella-riquetti.netlify.app](http://isabella-riquetti.netlify.app/)**
- 🔭 Currently leveling up my DevOps skills
- 🤝 Open to collaborating on open source projects

---

### 🎯 Goals — new chapter

I finished my last goal list, so here's what's next:

- [ ] Go deeper into Docker: multi-container apps with Docker Compose
- [ ] Set up a CI/CD pipeline for one of my projects
- [ ] Keep growing **[planye](https://planye.com/)** with new features
- [ ] Help small companies to have their own websites
- [ ] Win Shipathon
- [ ] Attend or speak at a tech meetup

<details>
<summary>✅ Previously completed goals</summary>
<br>

- [x] Participate in a Hackathon
- [x] Contribute to open source
- [x] Learn basic TypeScript
- [x] Learn more about Docker
- [x] Create a personal website
- [x] Share my GitLab metrics here in my README
- [x] Create a basic chat using webhooks
- [x] Use webhooks to build a change-notification system
- [x] Build a portfolio
- [x] Ship a full working website — [habity.care](https://habity.care/)

</details>

---

### 🛠️ Tech stack

<div align="center">

![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)

</div>

---

### 🚀 Featured project

<div align="center">
  
#### [planye](https://planye.com/)
A full working web app currently already with users that was created as a full replacement for phisical planners.

#### [habity.care](https://habity.care/)
A full working website I built end-to-end — one of my proudest projects so far using AI to enhance beauty routines.

</div>

---

<div align="center">

*Thanks for stopping by! Feel free to explore my repos, or say hi on my [personal site](http://isabella-riquetti.netlify.app/)!* 🐾

![Profile views](https://komarev.com/ghpvc/?username=isabella-riquetti&color=A78BFA&style=flat)

</div>
